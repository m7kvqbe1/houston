...
class System
{
	public static function generateAssets(Request $request, Application $app)
	{
		...
		if(!$app['session']->get('isAuthenticated')) {
			$template = file_get_contents(DOCUMENT_ROOT.'/src/assets/backbone/loginapp/index'.$env.'.html');
		} else {
			$template = file_get_contents(DOCUMENT_ROOT.'/src/assets/backbone/ticketapp/index'.$env.'.html');
		}
